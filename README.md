...
### ステップ12 可変長の引数 (改善編)
* add メソッドや square メソッドを追加し、計算を行いやすく改善。
...
### ステップ16 複雑な計算グラフ (実装編)
* 世代を追加
* 世代順に計算グラフを構築するため、優先度付きキューを採用
* FunctionExecutor トレートオブジェクトを世代順に並べるため、以下のように工夫
  * FunctionExecutor に対して PartialEq、Eq、PartialOrd、Ord を実装
...
* Variable 以外の数値とのオーバーロードに対応した。ただし数値のみ。
* 書籍に記載のある ndarray や左項の対応はこれから検討する。
