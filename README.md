...
### ステップ37 テンソルを扱う
* スカラ値だけでなく行列に対する基本的な演算を行ってみる。
* テンソルに関する机上学習

### ステップ38 形状を変える関数
* 逆伝播で使用する微分値の形状を、入力値と同じ形状にするように対応した。
* リシェイプと転置関数を追加し、Variable の関数として実行できるように対応した。
* 軸の順番を指定した転置にも対応した。
...
### ステップ42 線形回帰
* 乱数生成に ndarray-rand (0.15.0) を導入した。
  ndarray-rand が依存している rand のバージョンが 0.8 系であったため、それまで使用していた rand 0.9 系から 0.8 系に変更し、
...
* 順伝播、逆伝播に対応したシグモイド関数を追加し、グラフ描画を実装した。
...
* オプティマイザ用のトレイトとして Optimizer を追加した。
* オプティマイザの実装として SGD を追加し、これまで直接実装していた勾配の更新処理を SGD を使うように修正した。
* 書籍はオプティマイザ側で全てのパラメータを更新する実装となっているが、こちらはレイヤモデル側で更新する処理を実装している (LayerModel::update_parameters)。
* オプティマイザの hook については用途が判明しないため実装を保留する。
* MomentumSgd を追加した。
...
### ステップ48 多値分類
* スパイラルデータセットを生成する関数を実装し、グラフ描画した。
* スパイラルデータの学習と決定境界のグラフ描画を実装した。

### ステップ49 Dataset クラスと前処理 / ステップ50 ミニバッチを取り出す DataLoader
